...
public class ResourceManager extends EntityManager<Resource, Long> {
    ...
    public List<Resource> getResourcesByUserId(Long userId) throws DaoException,
        ...
        //角色权限
        List<Resource> roleResources = resourceDao.distinct(resourceDao.createQuery("select ms from User u left join u.roles rs left join rs.resources ms where u.id= :p1 order by ms.orderNo asc", userId)).list();
        //用户直接权限
        User user = userManager.loadById(userId);
   ...
   @Cacheable(value = { CacheConstants.RESOURCE_USER_RESOURCE_TREE_CACHE},key = "#userId +'getResourceTreeByUserId'")
     ...
     * @throws ServiceException
     */
    public TreeNode getTreeNode(Resource entity, Long excludeReourceId, boolean isCascade)
            throws DaoException, SystemException, ServiceException {
        TreeNode node = this.resourceToTreeNode(entity,null,false);
	...
	public List<Resource> getByParentId(Long parentId, Integer status)
		...
		hql.append("from Resource r where r.status = :status  ");
